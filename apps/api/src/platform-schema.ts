...
export async function ensurePlatformSchema() {
  ...
  try {
    await pool.query(`
      ...
      CREATE TABLE IF NOT EXISTS api_tokens (id uuid PRIMARY KEY,user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,name text NOT NULL,token_hash text NOT NULL UNIQUE,scopes jsonb NOT NULL DEFAULT '[]'::jsonb,expires_at timestamptz,last_used_at timestamptz,created_at timestamptz NOT NULL DEFAULT now());
      ...
      CREATE INDEX IF NOT EXISTS recovery_points_project_idx ON recovery_points(project_id,created_at DESC);
      CREATE TABLE IF NOT EXISTS application_graph_nodes (id uuid PRIMARY KEY,project_id uuid NOT NULL REFERENCES projects(id) ON DELETE CASCADE,service_id uuid REFERENCES services(id) ON DELETE CASCADE,node_key text NOT NULL,name text NOT NULL,type text NOT NULL,status text NOT NULL DEFAULT 'unknown',metadata jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),UNIQUE(project_id,node_key));
      ...
      CREATE TABLE IF NOT EXISTS autoscaling_decisions (id uuid PRIMARY KEY,service_id uuid NOT NULL REFERENCES services(id) ON DELETE CASCADE,current_replicas integer NOT NULL,desired_replicas integer NOT NULL,reason text NOT NULL,metrics jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now());
