...
app.post('/api/v1/deployments/:id/status', async (req, reply) => {
  if (ENGINE_CALLBACK_SECRET) {
    const supplied = req.headers['x-engine-secret']?.toString() ?? '';
    if (supplied !== ENGINE_CALLBACK_SECRET) return reply.code(401).send({ error: 'Unauthorized engine callback' });
  }
  const id = (req.params as { id: string }).id;
  const body = z.object({
    status: z.enum(['queued', 'building', 'starting', 'ready', 'failed', 'rolling_back']),
...
app.post('/api/v1/deployments', async (req, reply) => {
  ...
  try {
    if (body.repo) {
      await updateDeployment(id, 'building');
    ...
    const runtimeResponse = await fetch(`${ENGINE_URL}/api/v1/runtime/deploy`, {
      method:'POST',
      headers:{'content-type':'application/json', 'Idempotency-Key': `deployment:${id}`},
      body:JSON.stringify({ deploymentId:id, name:`nexus-${id.slice(0,8)}`, image, containerPort:body.containerPort, hostPort:body.hostPort, healthPath:body.healthPath, env:body.env }),
    });
