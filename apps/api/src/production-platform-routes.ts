...
import { z } from 'zod';
import pg from 'pg';
import { addDeploymentEvent, createRecoveryPoint, getApplicationGraph, getDeploymentState, listDeploymentEvents, listRecoveryPoints, recordAutoscalingDecision, updateDeployment, updateDeploymentState, upsertGraphEdge, upsertGraphNode } from './db.js';
...
async function claimRecoverableDeployments(requestedNodeName?: string) {
  ...
  try {
    ...
    const healthyNodes = nodes.rows.filter((n:any)=>Number(n.capacityCpuMillis)-Number(n.usedCpuMillis)>0&&Number(n.capacityMemoryBytes)-Number(n.usedMemoryBytes)>0);
    ...
    for(const deployment of result.rows){
      ...
      const originalNode=String((metadata as any).recoveryOriginalNodeName??originalSpec.originalNodeName??originalSpec.nodeName??'local');
      const existingRecovery=String((metadata as any).recoveryNodeName??'');
      const existingHealthy=existingRecovery&&healthyNodes.find((n:any)=>String(n.name)===existingRecovery);
      let target:any=existingHealthy||healthyNodes.find((n:any)=>String(n.name)===originalNode);
      if(!target){
      ...
      const deploymentSpec={...originalSpec,nodeName:targetName,originalNodeName:originalNode};
      const alreadyClaimed=existingRecovery===targetName&&existingHealthy;
      const nextMetadata=alreadyClaimed?metadata:{...metadata,deploymentSpec,recoveryNodeName:targetName,recoveryLeaseUntil:leaseUntil,recoveryOriginalNodeName:originalNode,recoveryClaimedAt:new Date().toISOString()};
      if(!alreadyClaimed)await client.query(`UPDATE deployment_state SET metadata=$2::jsonb,updated_at=NOW() WHERE deployment_id=$1`,[deployment.id,JSON.stringify(nextMetadata)]);
      claimed.push({...deployment,metadata:nextMetadata,recoveryNodeName:targetName});
    }
