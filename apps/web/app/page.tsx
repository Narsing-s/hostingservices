...
type Deployment={id:string;status:string;repo?:string;runtime?:{port?:number;url?:string;host?:string;tls?:boolean};createdAt:string};
...
export default function Home(){
 ...
 const [projectName,setProjectName]=useState('my-app'); const [repo,setRepo]=useState(''); const [ref,setRef]=useState('main'); const [containerPort,setContainerPort]=useState('3000'); const [hostPort,setHostPort]=useState('8088'); const [domain,setDomain]=useState(''); const [healthPath,setHealthPath]=useState('/');
 ...
 async function deploy(){
  ...
  try{
   ...
   if(domain.trim()){
    ...
    if(!dr.ok) throw new Error(d.detail||d.error||'Domain registration failed');
    throw new Error(`Domain registered but not verified. Add TXT ${d.verification.name} = ${d.verification.value}, then verify it with POST /api/v1/domains/${d.id}/verify before deploying.`);
   }
   setMessage(repo?'Building repository…':'Starting runtime…');
   const d=await fetch(`${API}/api/v1/deployments`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({projectId:project.id,repo:repo||undefined,ref:ref||'main',image:repo?undefined:'nginx:alpine',hostPort:Number(hostPort)||undefined,containerPort:Number(containerPort)||80,healthPath:healthPath||undefined})});
 ...
 return <main>
  ...
  <section className="content"><header><div><span className="eyebrow">CONTROL PLANE / PRODUCTION</span><h1>Your infrastructure, without the infrastructure work.</h1><p>Ship code, databases and workers as one connected application. Nexus detects, deploys and watches the whole graph.</p></div><button onClick={()=>setOpen(true)}><Rocket size={17}/>New deployment</button></header>
   ...
   <div className="terminal"><div className="termHead"><span><Terminal size={14}/> live deployment stream</span><span>production / runtime</span></div>{deployments.length?deployments.slice(0,4).map(d=><div key={d.id}><em>{new Date(d.createdAt).toLocaleTimeString()}</em> <b>{d.status.toUpperCase()}</b> {d.repo||'container deployment'}{d.runtime?.url?` → ${d.runtime.url}`:''}</div>):<><div><em>09:41:02</em> <b>PLAN</b> Environment graph resolved</div><div><em>09:41:08</em> <b>PULL</b> Container image resolved</div><div><em>09:41:10</em> <b>RUN</b> Docker runtime connected</div><div><em>09:41:11</em> <b>READY</b> Deployment health check passed <i/></div></>}</div>
  </section>
 </main>;
}
