...
function Panel({title,description,children}:{title:string;description:string;children:React.ReactNode}){return <div className="panel"><div className="panelHead"><div><span className="eyebrow">NEXUS CONTROL</span><h2>{title}</h2><p>{description}</p></div></div>{children}</div>}
function Empty({text}:{text:string}){return <div className="empty"><Settings2 size={18}/><span>{text}</span></div>}
