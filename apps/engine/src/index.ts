...
function readBody(req: http.IncomingMessage): Promise<string> {
  return new Promise((resolve, reject) => {
    let body = '';
    req.on('data', (chunk) => { body += chunk; if (body.length > 1_000_000) req.destroy(new Error('request body too large')); });
...
async function main(req: http.IncomingMessage, res: http.ServerResponse) {
  try {
    ...
    if (req.url === '/api/v1/runtime/build' && req.method === 'POST') {
      ...
      if (!body.repo || !body.image) { send(res, 400, { error: 'repo and image are required' }); return; }
      send(res, 200, await buildFromGit(body)); return;
    }
    ...
    if (req.url?.startsWith('/api/v1/runtime/logs/')) {
      ...
      send(res, 200, { name, logs: logs.toString() }); return;
    }
