...
export type RuntimeSpec = { name: string; image: string; containerPort?: number; hostPort?: number; env?: Record<string, string>; healthPath?: string };
...
async function ensureNetwork(docker: Docker) {
  ...
  try { await existing.inspect(); return existing; } catch { return docker.createNetwork({ Name: name, Driver: 'bridge' }); }
}

async function removeContainer(docker: Docker, name: string) {
  try { const container = docker.getContainer(name); await container.stop({ t: 10 }).catch(() => undefined); await container.remove({ force: true }).catch(() => undefined); } catch { /* not found */ }
}
...
async function createAndStart(docker: Docker, spec: RuntimeSpec, containerName: string, hostPort: number) {
  ...
  const container = await docker.createContainer({
    ...
    HostConfig: { RestartPolicy: { Name: 'unless-stopped' }, PortBindings: { [`${port}/tcp`]: [{ HostPort: String(hostPort > 0 ? hostPort : '') }] } },
...
async function findActiveContainer(docker: Docker, runtimeName: string) {
  const containers = await docker.listContainers({ all: true, filters: { label: [`nexus.runtime=${runtimeName}`, 'nexus.managed=true'] } });
  const active = containers.find((item) => item.State === 'running' && !item.Names.some((name) => name.includes('-candidate-')));
  return active ?? containers.find((item) => item.State === 'running');
}
...
export async function deployRuntime(spec: RuntimeSpec) {
  ...
  try {
    const previous = proxyEnabled ? await findActiveContainer(docker, spec.name) : (() => { try { return docker.getContainer(spec.name); } catch { return undefined; } })();
    ...
    if (proxyEnabled) {
      ...
      // The route now points at the healthy candidate. Only then retire the old container.
      if (previousName) await removeContainer(docker, previousName);
      return { id: candidate.id, name: candidateName, url: route.url, host: route.host, health, replacedContainerId: previousId, candidateHealth: health };
    }
    ...
    if (previousName) {
      ...
      } catch (error) { await removeContainer(docker, previousName); throw error; }
    }
