...
export async function deployRuntime(spec: RuntimeSpec) {
  ...
  const publicService = spec.public !== false;
  const port = spec.containerPort ?? 80;
  const requestedHostPort = spec.hostPort ?? port;
  const proxyEnabled = publicService && process.env.TRAEFIK_ENABLED !== 'false';
  ...
  try {
    ...
    const health = await waitForHealthyContainer(candidateName, { httpUrl: healthUrl, mode: spec.healthMode });
    ...
    return { id: candidate.id, name: spec.name, url: publicService && actualCandidatePort ? `http://localhost:${actualCandidatePort}` : undefined, health, replacedContainerId: previousId, public: publicService };
  } catch (error) {
