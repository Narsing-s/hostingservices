...
async function packageJson(dir: string): Promise<any | undefined> {
  try { return JSON.parse(await readFile(path.join(dir, 'package.json'), 'utf8')); } catch { return undefined; }
}

function nodeDockerfile(pkg: any, staticSite: boolean) {
  ...
  const build = pkg?.scripts?.build ? (manager === 'pnpm' ? 'pnpm run build' : manager === 'yarn' ? 'yarn build' : 'npm run build') : '';
  if (staticSite) {
    return `FROM node:22-bookworm-slim AS build\nWORKDIR /app\nCOPY package*.json ./\nCOPY pnpm-lock.yaml* yarn.lock* package-lock.json* ./\nRUN ${install}\nCOPY . .\nRUN ${build || 'npm run build'}\nFROM nginx:1.27-alpine\nCOPY --from=build /app/dist /usr/share/nginx/html\nCOPY --from=build /app/build /tmp/build-output\nRUN if [ -d /tmp/build-output ] && [ ! -d /usr/share/nginx/html/assets ]; then cp -r /tmp/build-output/* /usr/share/nginx/html/; fi\nEXPOSE 80\nCMD ["nginx", "-g", "daemon off;"]\n`;
  }
...
async function detectDockerfile(dir: string) {
  ...
  if (pkg) {
    ...
    const staticSite = Boolean(await exists(path.join(dir, 'vite.config.ts')) || await exists(path.join(dir, 'vite.config.js')) || await exists(path.join(dir, 'astro.config.mjs')) || await exists(path.join(dir, 'angular.json')) || (!scripts.start && scripts.build && !pkg.dependencies?.express && !pkg.dependencies?.fastify && !pkg.dependencies?.next));
    await writeFile(path.join(dir, 'Dockerfile'), nodeDockerfile(pkg, staticSite));
  ...
  if (await exists(path.join(dir, '*.csproj'))) {
  ...
  if (await exists(path.join(dir, 'Gemfile'))) {
    await writeFile(path.join(dir, 'Dockerfile'), `FROM ruby:3.4-slim\nWORKDIR /app\nCOPY Gemfile Gemfile.lock* ./\nRUN bundle install\nCOPY . .\nENV PORT=3000\nEXPOSE 3000\nCMD ["sh", "-c", "bundle exec ${'${RUBY_START_COMMAND:-rails server -b 0.0.0.0 -p 3000}'}"]\n`);
