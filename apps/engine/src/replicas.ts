...
async function createReplica(docker: Docker, spec: ReplicaSpec, name: string, index: number) {
  ...
  return docker.createContainer({
    name, Image: spec.image, Env: Object.entries(spec.env ?? {}).map(([k, v]) => `${k}=${v}`), Cmd: spec.command,
    ExposedPorts: { [`${port}/tcp`]: {} },
    HostConfig: { RestartPolicy: { Name: 'unless-stopped' }, PortBindings: { [`${port}/tcp`]: [{ HostPort: '0' }] } },
    NetworkingConfig: { EndpointsConfig: { [process.env.NEXUS_RUNTIME_NETWORK ?? 'nexus-runtime']: {} } },
    Labels: { 'nexus.managed': 'true', 'nexus.runtime': spec.name, 'nexus.replica': 'true', 'nexus.replica-index': String(index), 'nexus.public': String(spec.public !== false) }
  });
}
...
export async function deployReplicas(spec: ReplicaSpec) {
  ...
  } catch (error) { for (const container of started) await remove(docker, container.id); throw error; }
}
...
async function averageCpuPercent(docker: Docker, containers: Docker.ContainerInfo[]) {
  ...
  const values = await Promise.all(containers.map(async (item) => {
    try {
      const raw = await docker.getContainer(item.Id).stats({ stream: false });
      const stats: any = Buffer.isBuffer(raw) ? JSON.parse(raw.toString()) : raw;
      ...
      return systemDelta > 0 ? (cpuDelta / systemDelta) * cpus * 100 : 0;
    } catch { return 0; }
  }));
...
async function reconcileScale(spec: ReplicaSpec, target: number) {
  ...
  const bounded = Math.max(1, Math.min(20, target));
  if (bounded === current) return current;
  if (bounded > current) {
    for (let i = current; i < bounded; i++) {
      const name = `${spec.name}-replica-${i + 1}`;
      const c = await createReplica(docker, spec, name, i + 1); await c.start();
      const hp = await hostPort(c, port); const httpUrl = spec.healthPath && hp ? `http://127.0.0.1:${hp}${spec.healthPath}` : undefined;
      await waitForHealthyContainer(name, { mode: spec.healthMode ?? (httpUrl ? 'http' : 'process'), httpUrl, timeoutMs: 120_000 });
    }
  } else {
    const removable = items.sort((a, b) => Number(b.Labels?.['nexus.replica-index'] ?? 0) - Number(a.Labels?.['nexus.replica-index'] ?? 0)).slice(0, current - bounded);
    for (const item of removable) await remove(docker, item.Names?.[0]?.replace(/^\//, '') || item.Id);
  }
  if (spec.public !== false && process.env.TRAEFIK_ENABLED !== 'false') {
    const active = await docker.listContainers({ all: false, filters: { label: [`nexus.runtime=${spec.name}`, 'nexus.managed=true', 'nexus.replica=true'] } });
    await switchTraffic(spec.name, active.map((x) => x.Names?.[0]?.replace(/^\//, '') || x.Id), port, spec.healthPath, spec.domain);
  }
  return bounded;
}

export function startAutoscaler(spec: ReplicaSpec) {
  if (!spec.autoscale) return;
  const existing = controllers.get(spec.name); if (existing) clearInterval(existing);
  ...
  const tick = async () => {
    try {
      ...
      const cpu = await averageCpuPercent(docker, items);
      const now = Date.now();
      if (now - (lastScale.get(spec.name) ?? 0) < interval * 2) return;
      const current = items.length;
      let target = current;
      if (cpu > spec.autoscale!.cpuPercent + 10 && current < spec.autoscale!.max) target = current + 1;
      else if (cpu < spec.autoscale!.cpuPercent - 20 && current > spec.autoscale!.min) target = current - 1;
      if (target !== current) { await reconcileScale(spec, target); lastScale.set(spec.name, now); console.log(`[autoscaler] ${spec.name}: cpu=${cpu.toFixed(1)}% replicas ${current}->${target}`); }
    } catch (error) { console.error(`[autoscaler] ${spec.name}: ${error instanceof Error ? error.message : String(error)}`); }
  };
