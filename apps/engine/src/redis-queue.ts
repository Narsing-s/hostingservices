...
export async function enqueueDeployment(job: DeploymentJob, idempotencyKey?: string) { const jobId = idempotencyKey?.replace(/[^a-zA-Z0-9:_-]/g, '_').slice(0, 200); const existing = jobId ? await deploymentQueue.getJob(jobId) : undefined; if (existing) return existing; return deploymentQueue.add(job.operation, job, jobId ? { jobId } : undefined); }
export async function queueStats() { return deploymentQueue.getJobCounts('waiting', 'active', 'completed', 'failed', 'delayed'); }
export function createDeploymentWorker(handler: (job: Job<DeploymentJob>) => Promise<void>) { const worker = new Worker<DeploymentJob>('nexus-deployments', handler, { connection: connection(), concurrency: Number(process.env.DEPLOYMENT_CONCURRENCY ?? 2), limiter: { max: Number(process.env.DEPLOYMENT_RATE_LIMIT ?? 20), duration: 1000 } }); worker.on('completed', (job) => console.log(`[deployment] completed ${job.id}`)); worker.on('failed', (job, error) => console.error(`[deployment] failed ${job?.id}: ${error.message}`)); return worker; }
